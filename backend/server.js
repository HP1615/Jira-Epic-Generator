...
async function fetchTicketCodeContext(panelName, formPath, branch = 'develop') {
  ...
  if (Array.isArray(formPath)) {
    ...
    const isLikelyFileList = formPath.match(/\.(js|ts|jsx|tsx|json|html|css|cs|java|py|php|xml|yml|yaml|md)(\s*,\s*|\s*$)/i);
    if (isLikelyFileList) {
      fileNames = formPath.split(",").map(f => f.trim()).filter(Boolean);
    } else if (typeof searchSimilarFiles === "function") {
      // Use embeddings to find similar files
      const collectionName = `${repoName}_files`;
      fileNames = await searchSimilarFiles(formPath, collectionName, 3);
      console.log(`Found similar files via embeddings: ${fileNames.join(', ')}`);
    } else {
      // Fallback: treat as a single file name
      fileNames = [formPath];
    }
  }
